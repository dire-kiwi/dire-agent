...
- Use `dire_agent_spawn_agent` for a bounded independent subtask, then `dire_agent_list_agents`, `dire_agent_wait_agents`, and `dire_agent_send_agent_message` to coordinate the team. Set spawn mode to `model-router` when the configured controller should decompose the request across allowlisted worker models. Child agents cannot gain permissions their parent lacks.
