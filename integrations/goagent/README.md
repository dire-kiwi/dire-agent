# GoAgent Codex desktop plugin

This plugin launches the `goagent-mcp` companion, which connects to the local
daemon at `ws://127.0.0.1:7331/ws`. It does not use Codex app-server.

Build or install the companion so `goagent-mcp` is on the desktop app's PATH,
start `goagentd`, then add this directory as a local Codex plugin or marketplace
